...
import lzy.com.money.Activity.NotifyActivity;
...
public class FloatWindowService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        // 开启定时器，每隔0.5秒刷新一次
        mApp = (moneyApplication) getApplication();
        if (mApp.getSystemConfig(1)) {
            Intent notifyIntent = new Intent(this, NotifyActivity.class);
            notifyIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
                    | Intent.FLAG_ACTIVITY_CLEAR_TASK);
            // Creates the PendingIntent
            PendingIntent pendIntent = PendingIntent.getActivity(this, 99808962,
