package lzy.com.money.LockView;
...
public class LockPatternUtils {
	...
	private static class LockPatternFileObserver extends FileObserver {
		...
		public void onEvent(int event, String path) {
			...
			if ((moneyApplication.FileParent+"/"+LOCK_PATTERN_FILE).equals(path)) {
	...
	public LockPatternUtils() {
		if (sLockPatternFilename == null) {
			sLockPatternFilename = new File(moneyApplication.FileParent
					, LOCK_PATTERN_FILE);
			sHaveNonZeroPatternFile.set(sLockPatternFilename.length() > 0);
			...
			sPasswordObserver = new LockPatternFileObserver(
					moneyApplication.FileParent+"/"+LOCK_PATTERN_FILE, fileObserverMask);
	 ...
	 */
	public boolean saveLockPattern(List<LockPatternView.Cell> pattern) {
		// Compute the hash
